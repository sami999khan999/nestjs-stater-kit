...
export class AuthService {
  ...
  async signUp(dto: CreateUserDto) {
    ...
    // Hash password
    const hashPassword = await bcrypt.hash(dto.password as string, 12);
  ...
  async resendOtp(email: string, type: string) {
    ...
    const subject = subjectMap[type as keyof typeof subjectMap] || 'Your OTP';
  ...
  async resend2fa(email: string) {
    ...
    if (lastOtp) {
      const now = new Date();
      const isExpired = lastOtp.expiresAt && lastOtp.expiresAt < now;
      if (!isExpired) {
        throw new UnauthorizedException(
  ...
  async signInWeb(dto: UserLoginDto, req: Request, res: Response) {
    ...
    if (!user.isEmailVerified) {
      ...
      return {
        ...
        redirect: `${this.getAppClientUrl()}/auth/verify-email?token=${encodeURIComponent(
          token,
    ...
    if (user.isTwoFactorEnabled) {
      ...
      return {
        ...
        redirect: `${this.getAppClientUrl()}/auth/2fa?token=${encodeURIComponent(
          token,
        )}&email=${encodeURIComponent(user.email)}`,
      };
    }
  ...
  async logout(userId: string, res: Response) {
    ...
    const cookieOpts = { ...this.getCookieConfig(), path: '/' };
    ...
    res.cookie('refreshToken', '', { ...cookieOpts, maxAge: 0 });
    res.cookie('accessToken', '', { ...cookieOpts, maxAge: 0 });
  ...
  async verify2fa(otp: string, res: Response, req: Request) {
    ...
    return {
      status: true,
      redirect: `${this.getAppClientUrl()}${redirect}`,
